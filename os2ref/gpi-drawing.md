...
### `MATRIXLF` — a 3×3 transform [DOC-IBM — `os2def.h`]
...
|---|---|---|
| `GpiBitBlt` / `GpiWCBitBlt` (§10) | `GreBitblt` | Modifies bit-map data at a target rectangle in the current DC [DOC — EDM2 "GreBitblt"] |
| `GpiBox` (§6) | `GreBoxBoundary` | Draws a box from the current (X,Y) to the specified opposite corner [DOC — EDM2 "GreBoxBoundary"] |
| `GpiFullArc` (fill) (§5) | `GreFullArcInterior` | Draws a filled full arc using the current pattern, centred at the current position [DOC — EDM2 "GreFullArcInterior"] |
| `GpiBeginArea`…`GpiEndArea` (§7) | `GreAreaSetAttributes` | Sets the area/path fill attributes; engine-simulated, hooked by drivers that do their own area/path fill [DOC — EDM2 "GreAreaSetAttributes"] |
| `GpiModifyPath` (§7) | `GreModifyPath` | Modifies a path; returns BOOLEAN success [DOC — EDM2 "GreModifyPath"] |
| `GpiPathToRegion` / region create (§7) | `GreCreateRectRegion` | Creates a region as the OR of a series of rectangles [DOC — EDM2 "GreCreateRectRegion"] |
| region point test (§7) | `GrePtInRegion` | Point-in-region hit test (see §7) [DOC — EDM2 "GrePtInRegion"] |
| `GpiSet*TransformMatrix` concat (§4) | `GreMultiplyXforms` | Concatenates/overwrites a transform matrix (`SX_CAT_AFTER`/`SX_CAT_BEFORE`/`SX_OVERWRITE`/`SX_UNITY`) [DOC — EDM2 "GreMultiplyXforms"] |
| `GpiCreateLogFont` (§9) | `GreCreateLogicalFont` (+ `GreRealizeFont`) | Describes the logical font and returns the `FONT_MATCH`/`FONT_DEFAULT` match result; `GreRealizeFont` realizes it for the device [DOC — EDM2 "GreCreateLogicalFont", "GreRealizeFont"] |
| text extent / `GpiQueryTextBox` (§9) | `GreQueryTextBox` | Processes a string as if it were drawn and returns its bounding box [DOC — EDM2 "GreQueryTextBox"] |
| `GpiCreateLogColorTable` (§8) | `GreCreateLogColorTable` | Loads the logical color table (see §8) [DOC — EDM2 "GreCreateLogColorTable"] |
| `GpiQueryLogColorTable` (§8) | `GreQueryLogColorTable` | Reads back the logical color table (see §8) [DOC — EDM2 "GreQueryLogColorTable"] |
