...
split corroborated by the OS/2 for SMP book (§DosQuerySysInfo, `QSV_NUMPROCESSORS`
...
| 0x08 | `SIS_HrsTime` / `SIS_MinTime` / `SIS_SecTime` / `SIS_HunTime` | 4×UCHAR | hours / minutes / seconds / hundredths |
| 0x0C | `SIS_TimeZone` | USHORT | minutes from GMT (set to EST) |
| 0x0E | `SIS_ClkIntrvl` | USHORT | timer interval, units of 0.0001 s |
| 0x10 | `SIS_DayDate` / `SIS_MonDate` | 2×UCHAR | day (1–31) / month (1–12) |
| 0x12 | `SIS_YrsDate` | USHORT | year (≥ 1980) |
| 0x14 | `SIS_DOWDate` | UCHAR | day of week (1-1-80 = Tue = 3) |
| 0x15 | `SIS_VerMajor` / `SIS_VerMinor` / `SIS_RevLettr` | 3×UCHAR | major / minor version, revision letter (20 / 45 = Warp 4.5) |
...
| 0x20 | `SIS_MinSlice` / `SIS_MaxSlice` | 2×USHORT | minimum / maximum timeslice (ms) |
| 0x24 | `SIS_BootDrv` | USHORT | drive the system was booted from (1 = A) |
| 0x26 | `SIS_mec_table[32]` | 32 B | RAS Major Event Code table |
| 0x46 | `SIS_MaxVioWinSG` / `SIS_MaxPresMgrSG` | 2×UCHAR | max VIO-windowable / Presentation-Manager screen groups |
...
| Off | Field | Type | Meaning |
|---|---|---|---|
| 0x00 | `LIS_CurProcID` / `LIS_ParProcID` | 2×USHORT | current / parent process id |
| 0x04 | `LIS_CurThrdPri` / `LIS_CurThrdID` | 2×USHORT | current thread priority / thread id |
...
| 0x10 | `LIS_AX`…`LIS_DS` | 7×USHORT | .EXE-derived: environment selector, command-line offset, data-segment length, STACKSIZE, HEAPSIZE, module handle, data-segment handle |
| 0x1E | `LIS_PackSel` / `LIS_PackShrSel` / `LIS_PackPckSel` | 3×USHORT | first tiled / above-shared-arena / above-packed-arena selector |
