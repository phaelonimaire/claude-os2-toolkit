...
(e.g. the header field `strlength` precedes `strptr`; see §2).
...
- **`Exits`** — an array of `RXSYSEXIT` descriptors terminated by an `RXENDLST` code, enabling
  system exits for this run (§6); NULL if none.
...
(NULL `strptr`) makes `RC` = `"0"`. The default `Retstr` buffer is 256 bytes (§2 allocation rule
applies).
...
buffer, §2 allocation rule). Its **`ULONG` return value** is the outcome:
