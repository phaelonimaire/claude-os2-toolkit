...
`UniChar` and the object handle type is `UconvObject`; both are defined in §2.
...
sets* on the code-page side of a conversion (see the `esid` table, §5). [DOC-IBM `uconv.h`]
...
`UniCreateUconvObject`; `UniMapCpToUcsCp` (§7) builds that string from a numeric code page.
...
conversion attributes (substitution options, endianness, conversion type, display mask — see §6)
...
- `subst` returns the count of **non-identical conversions** — characters that had no exact mapping
  and were replaced by the substitution character (see the substitution options, §6). [DOC-IBM
...
reported in the `esid` field of the object attributes (§6) and is the primary check on whether a
...
| `esid` | `unsigned short` | Q | Encoding-scheme ID (§5) |
| `options` | `char` | Q/S | Substitution options (§6.2) |
| `state` | `char` | Q/S | State for stateful conversions |
| `endian` | `conv_endian_t` | Q/S | Source and target endianness (§6.3) |
| `displaymask` | `unsigned long` | Q/S | Display/data control mask (§6.4) |
| `converttype` | `unsigned long` | Q/S | Conversion type (§6.5) |
...
`subst` output, §4) or stops with `UCONV_EILSEQ`, per these bits:
...
pointer/count pairs of §4. `UniStrToUcs` converts the code-page string `source` to UCS-2 in
...
program takes the current code page from the NLS surface (§9) and feeds it through
...
- `DosQueryCtryInfo` / `COUNTRYINFO` and `DosQueryDBCSEnv` describe the country/code-page locale and
  ...
  `UniQueryUconvObject` (§6) describe for a specific conversion object.
...
> §3, and use the positive control: `grep -l Uni /usr/include/*.h`.
...
| Win32 | OS/2 |
|---|---|
| `MultiByteToWideChar` / `WideCharToMultiByte` | `UniUconvToUcs` / `UniUconvFromUcs` (§4), or `UniStrToUcs` / `UniStrFromUcs` (§7) |
...
and `GpiQueryTextBox` interpret the bytes you hand them in the **GPI** code page (§9.1), so UTF-8
passed straight through draws each byte as its own 8-bit glyph — `café` renders as `caf|®` — with no
...
- `config-and-environment.md` — NLS code-page surface (`DosQueryCp`,
  `DosQueryCtryInfo`, `DosQueryDBCSEnv`) for the §9 cross-reference.
