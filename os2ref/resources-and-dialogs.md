...
> **`codepage` — match it to the message queue** [DOC-IBM — `pm5.txt`, §"Code Pages"]. This field
...
> independent code-page scopes (process / queue / GPI); see `unicode-conversion.md` §9.1.
...
§4) or an application class name — and its `id` is what the dialog procedure and
...
(`MRESULT EXPENTRY DlgProc(HWND, ULONG, MPARAM, MPARAM)`; see `pm-window-messaging.md` §3) and must
...
Its parameters [DOC-IBM — PM Reference; and see `pm-window-messaging.md` §8]:
...
`WS_TABSTOP`/`WS_GROUP` styles on the items (see `pm-window-messaging.md` §4).
...
`pm-window-messaging.md` §8. `WinAlarm(HWND hwndDesktop, ULONG rgfType)` sounds a system alarm:
...
(the frame's menu-control id; see `pm-window-messaging.md` §4). An action-bar menu is created as a
...
stretching to `lWidth`×`lHeight`; `Resource` follows the same module convention. The bitmap is then
...
`PMSDMRI.DLL`. See `module-dll.md` §"Where the PM APIs actually live" before attempting any
...
owner is computed by the `WinLoadDlg` algorithm (§4). The return is the dialog `HWND` when
