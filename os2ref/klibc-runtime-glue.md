...
your `.EXE`'s `crt0.s`/`__init()` → `main()` → `exit()`; §6 below expands on it.
...
(`__LIBC_FHOPS`) that a custom backend (currently: BSD sockets — see §7) installed when the
...
32-bit-flat/high-memory boundary discussed in §5.
...
what to do with them. §7 shows the one custom backend actually present here, BSD sockets.
...
sockets (§7) — and fails with `EINVAL` on a plain OS/2 file, character device, or named-pipe
...
makes fd-passing across `spawn()`/`fork()` work — see §3 for why that indirection exists at
...
caveat as §8.2: not upstream, not cited here as real LIBC Next behavior.)
...
- fd inheritance goes through the explicit inherit-bundle mechanism in §2.5, not implicit
...
§4.2 only covers the signals an OS/2 *exception* can produce. The full POSIX signal space —
...
`_sys_expand_heap_obj_by()` at `heap.c:54`]. `OBJ_FORK` here matters for §3: memory objects the
...
the system at startup, per the `startup.c` init-order comment in §6) to decide whether to
enable the high-memory arena at all. §2.2's read/write bounce-buffer logic (devices reject
...
>     inherited from a parent — see §2.5/§3.4).
>   - calls `_sys_init_largefileio()` (checks for large-file APIs).
>   - calls `__libc_fhInit()` — initializes the fd table (§2).
...
>   - installs the exception handler and sets the signal focus (§4.1).
...
ordinary POSIX fd anyway, and it does so with exactly the fh-framework plug-in point §2
describes.
...
analogue of §3's forkable-module registration: a socket fd surviving `fork()` requires the
...
every one of §2's dispatch points (`__read`/`__write`/`__ioctl2`/`__select`/`close`) takes
...
implies it, per §2.1's general contract, but it wasn't read directly). One confirmed use of
...
Everything in §2 describes a *uniform* fd model: whatever the backend, `read()` on a kLIBC fd
...
- A process whose stdin (fd 0) has been redirected — to a pipe, a named pipe standing in for
  ...
  *does* go through §2's framework and *does* honor redirection).
...
`_scrsize()`. This is the mirror image of §2's fh-framework story: most of kLIBC generalizes
...
§8 covers the two calls that bypass the fd layer. This section covers the rest of what a program
...
| `select()` on a tty | only descriptors with a custom fh backend are selectable (§2.4) [SRC `sys/__select.c:118-123`] | `select(stdin)` returns `EINVAL` on a real console handle |
| `SIGWINCH` | `SPA_IGNORE`, comment "not implemented" [SRC `sys/signals.c:331`] | **resize can only ever be discovered by polling** (§4.3) |
...
`fork()` (§3) copies the child's memory, but **not** the parent's OS/2 kernel resources: a shared
...
(`pfnForkParent`/`pfnForkChild`, §2.1), called with `__LIBC_FORK_OP_FORK_CHILD` [SRC
...
- **`file-io.md`** — the native `DosOpen`/`DosRead`/`DosWrite`/`DosDevIOCtl` surface that
  backs a `pOps==NULL` fh (§2.2–2.3) and that `open()`/`sopen()` ultimately call into.
- **`process-thread.md`** — `DosExecPgm`/`DosCreateThread`/the TIB and PIB that §3's
  `fork()`/`spawn()` emulation and §6's `argv`/environ construction are built on.
- **`exceptions.md`** — the `DosSetExceptionHandler` chain and `XCPT_*` numbering that §4's
  signal emulation installs into and translates from.
- **`memory-api.md`** (+ `memory-model.md`) — `DosAllocMem`/`DosAllocMemEx`, `PAG_*`/`OBJ_*`
  flags, and the 512 MB flat-model boundary that §5's low/high heap split is built on.
- **`tcpip-sockets.md`** — the native BSD-style socket API §7 wraps in a kLIBC fd, including
  why an OS/2 socket isn't a Control Program handle in the first place.
- **`module-dll.md`** — `DosLoadModule`/`_DLL_InitTerm`, the mechanism §6 (per-DLL crt init)
  and §7 (dynamic resolution of the TCP/IP stack's socket entry points) both depend on.
- **`vio-kbd-mou.md`** — the native `Vio*`/`Kbd*`/`Mou*` API that §8's `_read_kbd()` and
