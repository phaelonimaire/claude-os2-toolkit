...
| `CreateDialogParam` (modeless) | `WinLoadDlg` + `WinShowWindow` | **no `IsDialogMessage` needed — §2.10** |
| `&` mnemonic in control text | `~` | **buttons and menus only — §2.11** |
...
| `WM_INITDIALOG` | `WM_INITDLG` (`0x003b`) | **return value is inverted — §2** |
...
- **`CenterWindow`** — do the arithmetic in bottom-left space throughout (see §2, origin).
...
Full detail: `os2ref/resources-and-dialogs.md` §5.
...
flips against its parent's** height. `os2ref/gpi-drawing.md` §"Coordinate origin".
...
code pages should match the queue's. `os2ref/unicode-conversion.md` §9.1.
...
substitute for §2.7; menu mnemonics still need the forwarding.
...
`os2ref/resources-and-dialogs.md` §4; OBS-RE — Notepad2's Find/Replace dialog.]
...
Full table and rules: `c-guide.md` §0.5.
...
coordinates are bottom-left. Verified example: `os2ref/resources-and-dialogs.md` §2.1.
...
4. **Read the failure's shape.** When only the code paths that bypass a subsystem work, the
   subsystem's state is the bug — that is how §2.5 was diagnosed from a single screenshot.
...
API" but "what did the caller actually need from it". Compare against §7.3, where the trap runs the
...
`os2-app-dev-guide.md` §3 describes, in its most expensive form: **grep this kit before concluding
